...
export const LossFunctions = {
   ...
   */
  mse(predictions: Tensor, targets: Tensor): { loss: Tensor, gradient: Tensor } {
   ...
   */
  crossEntropy(predictions: Tensor, targets: Tensor): { loss: Tensor, gradient: Tensor } {
    const predDense = predictions.type === 'sparse' ? toDense(predictions) : predictions
    const targetDense = targets.type === 'sparse' ? toDense(targets) : targets
   ...
   */
  binaryCrossEntropy(predictions: Tensor, targets: Tensor): { loss: Tensor, gradient: Tensor } {
    const predDense = predictions.type === 'sparse' ? toDense(predictions) : predictions
    const targetDense = targets.type === 'sparse' ? toDense(targets) : targets
